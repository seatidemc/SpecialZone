...
public class Commands implements TabExecutor {
    ...
    public Commands() {
        supportedProperties = Arrays.asList("keepInv", "keepExp", "noBreak", "ignoreY");
        zoneNames = Arrays.asList((String[]) Files.zones.getKeys(false).toArray());
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("specialzone")) {
            ...
            if (args.length == 2) {
                if (args[0] == "set" || args[0] == "delete" || args[0].endsWith("ex")) {
                    return getResult(args[1], zoneNames);
                }
            }
            if (args.length == 3) {
                if (args[0] == "set" || args[0] == "delex" || args[0] == "setex") {
            ...
            switch (args[0]) {
                case "create": {
                    ...
                    if (Zone.contains(args[1])) {
                        LogUtil.send(sender, "&c创建区域失败，区域已存在。");
                        return true;
                    }
                ...
                case "addex": {
                    ...
                    if (zone.setException(property, player)) {
                        LogUtil.send(sender, "成功将 &a" + player + "&r 加入区域 &e" + zoneName + "&r 的 &b" + property + "&r 属性排除项。");
                    } else {
                ...
                case "delex": {
                    ...
                    if (zone.delException(property, player)) {
                        LogUtil.send(sender, "成功从区域 &e" + zoneName + "&r 的 &b" + property + "&r 属性排除项移除 &a" + player + "&r。");
                    } else {
                ...
                case "getex": {
                    ...
                    if (result.size() == 0) {
                        ...
                        LogUtil.log(StringUtils.join(result, "、"));
                    }
                }
